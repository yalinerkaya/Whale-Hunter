...
public class TradeEventKafkaProducer implements KafkaProducerService<TradeEvent> {
    ...
    private final Logger logger = LoggerFactory.getLogger(TradeEventKafkaProducer.class);
    ...
    public void send(String key, TradeEvent data) {
        ...
        kafkaProducer.send(record, ((metadata, exception) -> {
            if (exception != null) {
                logger.error("레코드 입력 실패, 사유 :", exception);
            } else {
                logger.info("레코드 작성 성공! 파티션 : " + metadata.partition() + " 토픽 : " + metadata.topic());
            }
        }));
    }
